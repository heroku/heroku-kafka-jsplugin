...
describe('kafka:topics:info', () => {
  ...
  it('displays the topic info', async () => {
    ...
    expect(stdout).to.include('Producers: 0 messages/second (0 bytes/second) total')
    expect(stdout).to.include('Consumers: 0 bytes/second total')
    expect(stdout).to.include('Partitions: 3 partitions')
    expect(stdout).to.include('Replication Factor: 3')
    expect(stdout).to.include('Compaction: Compaction is disabled for topic-1')
    expect(stdout).to.include('Retention: 24 hours')
    api.done()
    kafka.done()
  })
  ...
  it('displays a topic prefix if one is specified', async () => {
    ...
    const {stdout} = await runCommand(TopicsInfo, ['topic-1', '--app', 'myapp'])
    expect(stdout).to.include('kafka-1 :: topic-1')
    expect(stdout).to.include('Topic Prefix: wisła-12345.')
    expect(stdout).to.include('Producers: 0 messages/second (0 bytes/second) total')
    api.done()
    kafka.done()
  })
